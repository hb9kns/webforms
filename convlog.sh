#!/bin/sh
# shell script for analyzing logfiles created with webforms
info='convlog.sh // 2018-04-12 Y.Bonetti // http://gitlab.com/yargo/webforms'
...
tmpr=${TMPDIR:-/tmp}/webform-$user-ana$$
...
usage: $0 [-m MM|-d NN] [c1 .. cK] beg=cL end=cM [cM+1 .. cN] [+cP]
 convert STDIN using columns <cL> and <cM> with absolute minute stamps,
 adding column behind <cM> with value=cM-cL, using maximum value MM or
 value=(1440-NN-cL) if <cM> is missing or undefined,
 and other columns <c1> to <cK> before and <cM+1> to <cN> after,
 and possibly all remaining columns starting from <cP>,
 print to STDOUT
